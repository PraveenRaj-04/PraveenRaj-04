## 👋 Hi, I'm **R T Praveenraj**

💻 Full Stack Developer | ☕ Java | 🌐 Web | 🤖 AI Enthusiast  
🚀 I build practical projects and learn by doing.

---

## 🧠 About Me

- 🔭 Currently building **full-stack web applications**
- 🌱 Learning **DSA, Spring Boot, and AI integration**
- 🛠️ Love turning ideas into working products
- 🎯 Focused on clean code, logic, and real-world use cases

---

## 🌐 Social & Professional Links

- 🔗 **LinkedIn**: [linkedin.com/in/praveenraj-r-t-1504673](https://www.linkedin.com/in/praveenraj-r-t-1504673)
- 📺 **YouTube**: [https://www.youtube.com/@DYKSHORTS](https://youtube.com/@dykshorts?si=uujKi0qZeevHd-6n)
- 📸 **Instagram**: [https://www.instagram.com/praveenraj_rt](https://www.instagram.com/praveenraj_rt?igsh=YjVtdDFmaG01b2o3)
- 📧 **Email**: raj81192004@gmail.com

---

## 🛠️ Tech Stack

### 👨‍💻 Languages
![Java](https://img.icons8.com/color/48/java-coffee-cup-logo.png)
![Python](https://img.icons8.com/color/48/python.png)
![JavaScript](https://img.icons8.com/color/48/javascript.png)

### 🎨 Frontend
![HTML](https://img.icons8.com/color/48/html-5.png)
![CSS](https://img.icons8.com/color/48/css3.png)
![Bootstrap](https://img.icons8.com/color/48/bootstrap.png)

### ⚙️ Backend
![Spring Boot](https://img.icons8.com/color/48/spring-logo.png)  
REST APIs • JDBC • J2EE

### 🗄️ Database
![MySQL](https://img.icons8.com/color/48/mysql-logo.png)
![MongoDB](https://img.icons8.com/color/48/mongodb.png)

### 🔧 Tools
![Git](https://img.icons8.com/color/48/git.png)
![GitHub](https://img.icons8.com/material-outlined/48/github.png)
![Postman](https://img.icons8.com/external-tal-revivo-color-tal-revivo/48/postman-api.png)
![Docker](https://img.icons8.com/color/48/docker.png)

---

## 🚀 Featured Projects

### 🛍️ Online Dress Shopping Web App
- Size-wise quantity selection
- Dynamic pricing & cart system  
**Tech:** HTML, CSS, JavaScript

---

### 💰 Money Manager Web App
- Track income & expenses
- Automatic balance calculation  
**Tech:** HTML, CSS, JavaScript

---

### 🛡️ AI-Based Cyber Threat Detection
- Detect malicious network traffic
- ML-based classification  
**Tech:** Python, Machine Learning

---

### 🦴 Bone Fracture Detection + Secure EMR
- X-ray based fracture detection
- Secure medical records system  
**Tech:** Python, Deep Learning

---

## 📌 Currently Working On

- ✔️ Strengthening **DSA & problem-solving**
- ✔️ Building projects using **Spring Boot**
- ✔️ Exploring **AI in web applications**

---

## ✨ Philosophy

> *Code. Break. Learn. Repeat.*
